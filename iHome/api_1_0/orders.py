...
@api.route(rule="/orders", methods=["POST"])
@login_required
def save_order():
    """保存订单"""
    user_id = g.user_id
...
@api.route(rule="/user/orders", methods=["GET"])
@login_required
def get_user_orders():
    """查询用户订单信息"""
    user_id = g.user_id
    ...
    try:
        ...
        current_app.logger.error(ex)
        return jsonify(errno=RET.DBERR, errmo="查询订单信息失败")
...
@api.route(rule="/orders/<int:order_id>/status", methods="PUT")
@login_required
def accept_reject_order(order_id):
    ...
    # action参数表名客户端请求的是接单还是拒单的行为
    action = req_data.get("action")
    if action not in ("accept", "reject"):
        return jsonify(errno=RET.PARAMERR, errmsg="参数错误")

    try:
        # 根据订单号查询订单，并且要求订单出去等待接单状态
...
@api.route(rule="/orders/<int:order_id>/comment", methods=["PUT"])
@login_required
