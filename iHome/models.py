...
class User(BaseModel, db.Model):
    ...
    def password(self, value):
        ...
        :return:
        """
        self.password_hash = generate_password_hash(password=value, method="pbkdf2:sha256", salt_length=8)
...
class House(BaseModel, db.Model):
    ...
    def to_basic_dict(self):
        ...
        house_dict = {
            ...
            "img_url": constains.FAST_DFS_URL + self.index_image_url if self.index_image_url else "",
            "room_count": self.room_count,
            "order_count": self.order_count,
            "address": self.address,
            "user_avatar": constains.FAST_DFS_URL + self.user.avatar_url if self.user.avatar_url else "",
    ...
    def to_full_dict(self):
        ...
        for image in self.images:
            img_urls.append(constains.FAST_DFS_URL + image.url)
        ...
        orders = Order.query.filter(Order.house_id == self.id, Order.status == "COMPLETE",
            ...
            constains.HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS)
        for order in orders:
            comment = {
